...
reg  BREADY_reg = 0;
reg [DATA_WIDTH - 1 : 0] mem_gain [gain_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_gain [ (gain_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * gain_DEPTH -1 : 0] = '{default : 'hz};
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            ...
            if (write_gain_run_flag === 1 && get_ack === 1) begin
                ...
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    ...
                    if(image_mem_gain[write_gain_count * four_byte_num  + i]!==gain_data_tmp_reg) begin
                    write (gain_data_in_addr + write_gain_count * four_byte_num * 4 + i * 4, gain_data_tmp_reg, write_gain_resp);
                    image_mem_gain[write_gain_count * four_byte_num + i]=gain_data_tmp_reg;
                    end
                end
                process_busy = 0;
      ...
      for(i = 0; i < gain_DEPTH; i = i + 1) begin 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_gain [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
