...
void find_hw_stub( char llvm_cbe_val,  char *llvm_cbe_in_vec,  char *llvm_cbe_out_vec);
void apatb_find_sw( char llvm_cbe_val,  char *llvm_cbe_in_vec,  char *llvm_cbe_out_vec);
...
printf("\n\{ BEGIN @find_hw_stub\n");
if (AESL_DEBUG_TRACE)
printf("\n  tail call void @find(i8 signext %%val, i8* %%in_vec, i8* %%out_vec), !dbg !3 for 0x%I64xth hint within @find_hw_stub  --> \n", ++aesl_llvm_cbe_174_count);
   /*tail*/ find(llvm_cbe_val, ( char *)llvm_cbe_in_vec, ( char *)llvm_cbe_out_vec);
if (AESL_DEBUG_TRACE) {
printf("\nArgument val = 0x%X",llvm_cbe_val);
}
  if (AESL_DEBUG_TRACE)
      printf("\nEND @find_hw_stub}\n");
  return;
...
printf("\n\{ BEGIN @apatb_find_sw\n");
if (AESL_DEBUG_TRACE)
printf("\n  tail call void @apatb_find_ir(i8 signext %%val, i8* %%in_vec, i8* %%out_vec) nounwind, !dbg !3 for 0x%I64xth hint within @apatb_find_sw  --> \n", ++aesl_llvm_cbe_182_count);
   /*tail*/ apatb_find_ir(llvm_cbe_val, ( char *)llvm_cbe_in_vec, ( char *)llvm_cbe_out_vec);
if (AESL_DEBUG_TRACE) {
printf("\nArgument val = 0x%X",llvm_cbe_val);
}
  if (AESL_DEBUG_TRACE)
